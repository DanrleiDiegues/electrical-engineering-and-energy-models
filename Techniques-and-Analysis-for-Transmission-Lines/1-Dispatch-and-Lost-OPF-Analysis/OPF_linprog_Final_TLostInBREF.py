...
def inpdat_final(DBAR, DLIN, DGER, PB=100.0, line_costs=None):
    ...
    for i in range(NGER):
        ...
        BARPG[i] = bus_to_pos[ger_bus]          # índice interno da barra do gerado
        PGMIN[i] = DGER[i, 1] / PB
        PGMAX[i] = DGER[i, 2] / PB
        CPG[i] = DGER[i, 3]
...
def OPF_Final(
    ...
    if hasattr(res, "eqlin") and res.eqlin is not None and res.eqlin.marginals is not None:
        ...
        #"lbd_BUS" significa "lambda_bus" e representa os preços nodais de energia, que indicam o valor marginal da energia em cada barra do sistema. Esses multiplicadores refletem a sensibilidade do custo total em relação a uma variação na demanda ou geração em cada barra, e são fundamentais para análises econômicas e operacionais do sistema de potência.
    ...
    if hasattr(res, "upper") and res.upper is not None and res.upper.marginals is not None:
        piup = -np.array(res.upper.marginals) # Multiplicadores dos limites superiores das variáveis, que indicam o quanto a função objetivo aumentaria se o limite superior de uma variável fosse relaxado. 
        # No contexto das linhas, esses multiplicadores estão relacionados à sensibilidade do custo em relação aos limites de fluxo nas linhas, e podem ser usados para avaliar o benefício de investir em reforços na capacidade das linhas.
    ...
    if piup is not None:
        ...
        print(f"Multiplicadores dos limites superiores de fdir (sens_fdir):\n{sens_fdir}\n")
        print(f"Multiplicadores dos limites superiores de frev (sens_frev):\n{sens_frev}\n")

        # sensibilidade agregada por linha
        sens_linha = np.maximum(sens_fdir, sens_frev)
        print(f"Sensibilidade agregada por linha (sens_linha):\n{sens_linha}\n")

        # índice benefício/custo
        indice_invest = sens_linha / CLINE
        print(f"Índice benefício/custo (indice_invest):\n{indice_invest}\n")
    '''
...
def OPF_Final_com_perdas_totais(
    DBAR, DLIN, DGER,
    PB=100.0,
    penalidade_pg=0.001,
    line_costs=None,
    ...
    - adicionando-a EXCLUSIVAMENTE na carga da barra de referência.
    ...
    PLOAD_eff = PLOAD_base.copy() # A carga efetiva é inicialmente igual à carga base, assumindo que não há perdas. O processo iterativo irá ajustar essa carga para incluir as perdas totais do sistema, convergindo para uma solução consistente onde a carga efetiva na barra de referência inclui as perdas calculadas com base nos ângulos de fase obtidos do OPF.
    historico = []
    ...
    for k in range(max_iter):
        ...
        # ================================================================
        # ========== ALLOCAÇÃO DE PERDAS NA BARRA DE REFERÊNCIA ==========
        # ================================================================
        ...
        if verbose:
            ...
            print(f"Carga efetiva na referência = {PLOAD_novo[BREF] * PB:.6f} MW")
            print(f"Erro = {erro}")
    ...
    if verbose:
        print("\n========== RESULTADOS FINAIS DO OPF COM PERDAS TOTAIS NA BARRA DE REFERÊNCIA ==========")
...
if __name__ == "__main__":
    ...
    resultado = OPF_Final_com_perdas_totais(
        DBAR,
        DLIN,
        DGER,
        PB=100.0,
