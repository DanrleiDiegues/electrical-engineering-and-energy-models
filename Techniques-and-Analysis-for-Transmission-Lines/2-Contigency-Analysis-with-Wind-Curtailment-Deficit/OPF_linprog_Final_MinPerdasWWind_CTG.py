...
import copy
import pickle # para backup/restauração de contingências
import joblib
import numpy as np
...
# pasta data no mesmo nível do script
...
# ============================================================
# CONFIGURAÇÃO DAS CONTINGÊNCIAS (CTG)
# ============================================================
# Ordem escolhida explicitamente pelo usuário.
# Exemplo: [1, 3, 5] significa:
#   - retira a linha 1, roda todos os cenários,
#   - devolve a linha 1,
#   - retira a linha 3, roda todos os cenários,
#   - devolve a linha 3,
#   - retira a linha 5, roda todos os cenários.
CTG_LINE_ORDER = [1, 3, 5]

# Para "retirar" a linha no modelo DC linear, o que realmente importa aqui é
# praticamente zerar o acoplamento angular da linha. Como o modelo usa Bor e G,
# fazemos Bor -> valor muito pequeno e G -> 0. Assim, o fluxo nessa LT tende a 0.
CTG_BOR_OFF = 1e-9
CTG_G_OFF = 0.0
CTG_R_MARK = 1e9
CTG_X_MARK = 1e9
...
def generate_load_scenario(PLOAD_b, QLOAD_b, rng, mode=None, config=None):
    """
    Gera um cenário de carga com base em uma variação global e local.
    
    Formulação inspirada base:
    PL_i = (1 + fg) * (1 + fl_i) * PL_base_i
    QL_i = (1 + fg) * (1 + fl_i) * QL_base_i

    Modos:
    - uniform: reproduz a formulação do slide
    - normal : usa distribuição normal para fator global e local
    """
    if config is None:
...
# ============================================================
# CONTINGÊNCIA DE LINHA
# ============================================================
def validate_ctg_line_order(line_order, nlin):
    ...
    for line_id in line_order:
        line_id = int(line_id)
        if line_id < 1 or line_id > nlin:
            raise ValueError(f"Linha {line_id} fora do intervalo válido 1..{nlin}.")
        validated.append(line_id)
    return validated


def apply_line_contingency(data, line_id):
    """
    Retira temporariamente uma linha do sistema, preservando os valores originais.
    line_id é 1-based, para bater com a numeração humana. Porque humanos gostam
    de contar começando em 1 e obrigam todo o resto do universo a se adaptar.
    """
    idx = int(line_id) - 1
    ...
    backup = {
        ...
        "x": float(data["x"][idx]),
        "G": float(data["G"][idx]),
        "Bor": float(data["Bor"][idx]),
    }

    data["r"][idx] = CTG_R_MARK
    data["x"][idx] = CTG_X_MARK
    data["G"][idx] = CTG_G_OFF
    data["Bor"][idx] = CTG_BOR_OFF

    return backup


def restore_line_contingency(data, backup):
    idx = backup["idx"]
    data["r"][idx] = backup["r"]
    data["x"][idx] = backup["x"]
    data["G"][idx] = backup["G"]
    data["Bor"][idx] = backup["Bor"]
...
def build_model_scenario(data, PLOAD_scn, PGW_scn, theta_prev=None):
    ...
    BARPG = data["BARPG"]
    PGMIN = data["PGMIN"]
    PGMAX = data["PGMAX"] # importante: inclui os geradores fictícios de déficit, que têm PGMIN=0 e PGMAX=valor alto
    CPG = data["CPG"] # custo de geração real + fictícia (deficit)
    ...
    # O lado direito do balanço nodal é a carga líquida, que já incorpora
    # a redução pela geração eólica disponível. Portanto, simplesmente atribuímos    
    BX[0:NBAR] = PNET

    # ============= CORTE DE VENTO =======================================
    # corte de vento entra como variável positiva que "devolve" carga líquida
    # AX(bus, pcw_i) = -1 para cada gerador eólico i conectado à barra "bus"
    for i in range(NGW):
        bus = NOGW[i]
...
def solve_opf_scenario(data, PLOAD_scn, PGW_scn, tol=1e-8, max_iter=50):
    ...
    for k in range(max_iter):
        ...
        idx = model["idx"]
        x = res.x # Variáveis de decisão: [PG_real_1, PG_real_2, ..., PG_fict_1, ..., theta_1, theta_2, ..., PCW_1, PCW_2, ...]
        # print(f"Iteração {k+1}: custo total = {res.fun:.4f}")
        # print(f"Iteração {k+1}: status = {res.message}")
        # print(f"Iteração {k+1}: número de variáveis = {len(x)}")
        # print(f"X: {x}")
        ...
        final = {
            ...
            "PG": PG,
            "THETA": THETA,
            "PCW": PCW, # Corte de Vento
            "perdas_linha": perdas_linha, # perdas por linha,    
            "fluxo": fluxo,
...
# ============================================================
def run_scenario_study_from_data(data, PB=100.0, Nc=1000, seed=123, ctg_line=None):
    """
    ...
    for ic in range(Nc):
        ...
        fob = sol["res"].fun
        perdas_mw = sol["perdas_totais"] * PB
        ...
        results.append({
            ...
            "PG_total_MW": PG * PB,
            "PCW_MW": sol["PCW"] * PB,
            "corte_carga_MW": corte_carga, # importante: corte de carga em MW, para análise de quanto da carga foi sacrificada em cada cenário
            "corte_vento_MW": corte_vento,
            "FOB": fob,
            "custo_pg": sol.get("custo_pg"),
            "custo_pcw": sol.get("custo_pcw"),
            "custo_pg_real": sol.get("custo_pg_real"), # importante: custo da geração real em $
            "custo_corte_carga": sol.get("custo_corte_carga"), # importante: custo do corte de carga em $
            "custo_corte_vento": sol.get("custo_corte_vento"), # importante: custo do corte de vento em $
            "perdas_MW": perdas_mw,
            "fluxo_MW": fluxo_mw,
            "theta_rad": sol["THETA"],
...
def run_scenario_study(DBAR, DLIN, DGER, DGW, PB=100.0, Nc=1000, seed=123, ctg_line=None):
    """
    This function runs a scenario study for the OPF problem with wind generation. 
    It generates multiple load and wind scenarios, solves the OPF for each scenario, 
    and collects results such as costs, losses, load shedding, wind curtailment, line flows, and dual variables. 
    ...
    return run_scenario_study_from_data(data, PB=PB, Nc=Nc, seed=seed, ctg_line=ctg_line)


# ============================================================
# ESTUDO DE CONTINGÊNCIAS
# ============================================================
def build_ctg_generator_ranking_tables(ctg_study):
    records_up = []
    records_down = []

    for item in ctg_study["ctg_results"]:
        line_id = item["ctg_linha"]
        MVu = np.asarray(item["summary"]["MVu_MW"])
        MVd = np.asarray(item["summary"]["MVd_MW"])

        for i, val in enumerate(MVu, start=1):
            records_up.append({
                "ctg_linha": line_id,
                "gerador": f"G{i}",
                "MVu_MW": float(val),
        ...
        for i, val in enumerate(MVd, start=1):
            records_down.append({
                "ctg_linha": line_id,
                "gerador": f"G{i}",
                "MVd_MW": float(val),
    ...
    df_up = pd.DataFrame(records_up).sort_values(["MVu_MW", "ctg_linha"], ascending=[False, True]).reset_index(drop=True)
    df_down = pd.DataFrame(records_down).sort_values(["MVd_MW", "ctg_linha"], ascending=[False, True]).reset_index(drop=True)
    return df_up, df_down


def run_contingency_study(DBAR, DLIN, DGER, DGW, PB=100.0, Nc=1000, seed=123, line_order=None):
    base_data = inpdat_opf(DBAR, DLIN, DGER, DGW, PB=PB)
    line_order = validate_ctg_line_order(line_order, base_data["NLIN"])

    if len(line_order) == 0:
        raise ValueError("O vetor de contingências está vazio. Informe ao menos uma linha.")

    ctg_results = []
    rows = []

    for line_id in line_order:
        data_ctg = copy.deepcopy(base_data)
        backup = apply_line_contingency(data_ctg, line_id)

        study_ctg = run_scenario_study_from_data(
            data_ctg,
            PB=PB,
            Nc=Nc,
            seed=seed,
        ...
        restore_line_contingency(data_ctg, backup)

        summary = study_ctg["summary"]
        rows.append({
            "ctg_linha": line_id,
            "corte_carga_medio_MW": summary["corte_carga_medio_MW"],
            "corte_vento_medio_MW": summary["corte_vento_medio_MW"],
            "custo_pg_real_medio": summary["custo_pg_real_medio"],
            "custo_corte_carga_medio": summary["custo_corte_carga_medio"],
            "custo_corte_vento_medio": summary["custo_corte_vento_medio"],
            "perdas_medias_MW": summary["perdas_medias_MW"],
            "FOB_media": summary["FOB_media"],
            "max_MVu_MW": float(np.max(summary["MVu_MW"])) if len(summary["MVu_MW"]) else 0.0,
            "max_MVd_MW": float(np.max(summary["MVd_MW"])) if len(summary["MVd_MW"]) else 0.0,
            "soma_MVu_MW": float(np.sum(summary["MVu_MW"])) if len(summary["MVu_MW"]) else 0.0,
            "soma_MVd_MW": float(np.sum(summary["MVd_MW"])) if len(summary["MVd_MW"]) else 0.0,
            "linha_mais_onerosa_pos_ctg": summary["linha_mais_onerosa"],
            "linha_mais_violada_pos_ctg": summary["linha_mais_violada"],
            "linha_maior_fluxo_abs_pos_ctg": summary["linha_maior_fluxo_abs"],
        })
    ...
    df_ctg = pd.DataFrame(rows)
    df_rank_corte_carga = df_ctg.sort_values("corte_carga_medio_MW", ascending=False).reset_index(drop=True)
    df_rank_curtailment = df_ctg.sort_values("corte_vento_medio_MW", ascending=False).reset_index(drop=True)
    df_rank_MVu = df_ctg.sort_values("max_MVu_MW", ascending=False).reset_index(drop=True)
    df_rank_MVd = df_ctg.sort_values("max_MVd_MW", ascending=False).reset_index(drop=True)
    df_MVu_gerador, df_MVd_gerador = build_ctg_generator_ranking_tables({"ctg_results": ctg_results})

    return {
        "line_order": line_order,
        "ctg_results": ctg_results,
        "ranking_ctg": df_ctg,
        "ranking_corte_carga": df_rank_corte_carga,
        "ranking_curtailment": df_rank_curtailment,
        "ranking_MVu": df_rank_MVu,
        "ranking_MVd": df_rank_MVd,
        "ranking_MVu_por_gerador": df_MVu_gerador,
        "ranking_MVd_por_gerador": df_MVd_gerador,
    }
...
def print_contingency_summary(ctg_study, top_n=10):
    print("\n" + "=" * 90)
    print("RESUMO DA ANÁLISE DE CONTINGÊNCIAS (CTG)")
    print("=" * 90)
    print(f"Ordem das linhas retiradas: {ctg_study['line_order']}")

    print("\nRanking por corte de carga médio:")
    print(ctg_study["ranking_corte_carga"].head(top_n).to_string(index=False))

    print("\nRanking por curtailment médio:")
    print(ctg_study["ranking_curtailment"].head(top_n).to_string(index=False))

    print("\nRanking por maior MVu:")
    print(ctg_study["ranking_MVu"].head(top_n).to_string(index=False))

    print("\nRanking por maior MVd:")
    print(ctg_study["ranking_MVd"].head(top_n).to_string(index=False))
...
def export_contingency_study_to_excel(ctg_study, filename="opf_ctg_results.xlsx"):
    df_ctg = ctg_study["ranking_ctg"].copy()
    df_carga = ctg_study["ranking_corte_carga"].copy()
    df_curt = ctg_study["ranking_curtailment"].copy()
    df_mvu = ctg_study["ranking_MVu"].copy()
    df_mvd = ctg_study["ranking_MVd"].copy()
    df_mvu_g = ctg_study["ranking_MVu_por_gerador"].copy()
    df_mvd_g = ctg_study["ranking_MVd_por_gerador"].copy()
    tables = build_ctg_final_tables(ctg_study)

    with pd.ExcelWriter(filename, engine="openpyxl") as writer:
        df_ctg.to_excel(writer, sheet_name="ctg_resumo", index=False)
        df_carga.to_excel(writer, sheet_name="ranking_corte_carga", index=False)
        df_curt.to_excel(writer, sheet_name="ranking_curtailment", index=False)
        df_mvu.to_excel(writer, sheet_name="ranking_MVu", index=False)
        df_mvd.to_excel(writer, sheet_name="ranking_MVd", index=False)
        df_mvu_g.to_excel(writer, sheet_name="MVu_por_gerador", index=False)
        df_mvd_g.to_excel(writer, sheet_name="MVd_por_gerador", index=False)
        tables["df_ctg_scalar"].to_excel(writer, sheet_name="ctg_scalar", index=False)
        tables["df_ctg_corte"].to_excel(writer, sheet_name="ctg_x_corte", index=False)
        tables["df_ctg_curt"].to_excel(writer, sheet_name="ctg_x_curtail", index=False)
        tables["df_ctg_mvu"].to_excel(writer, sheet_name="ctg_x_MVu", index=False)
        tables["df_ctg_mvd"].to_excel(writer, sheet_name="ctg_x_MVd", index=False)
        tables["df_ctg_mvu_gen"].to_excel(writer, sheet_name="ctg_MVu_gerador", index=False)
        tables["df_ctg_mvd_gen"].to_excel(writer, sheet_name="ctg_MVd_gerador", index=False)

    print(f"Arquivo Excel CTG salvo com sucesso: {filename}")
...
def plot_line_ranking_by_lambda(study, ctg_label="CTG", tol=0.0):
    summary = study["summary"]
    ...
    # filtra apenas lambdas diferentes de zero
    df_rank = df_rank[np.abs(df_rank["lambda_medio"]) > tol]

    # ordena
    df_rank = df_rank.sort_values("lambda_medio", ascending=True)

    # evita plot vazio
    if df_rank.empty:
        print(f"{ctg_label}: nenhuma linha com lambda ≠ 0")
        return
    ...
    fig.update_layout(
        title=f"Ranking das Linhas por Coeficiente de Lagrange Médio • {ctg_label}",
...
def build_ctg_final_tables(ctg_study):
    """
    Monta tabelas finais consolidadas para relatório:
    - CTG x corte de carga médio
    - CTG x curtailment médio
    - CTG x MVu máximo
    - CTG x MVd máximo
    - CTG x MVu por gerador
    - CTG x MVd por gerador
    """

    ctg_results = ctg_study["ctg_results"]

    rows_scalar = []
    rows_mvu_gen = []
    rows_mvd_gen = []

    for ctg in ctg_results:
        line_id = ctg["ctg_linha"]
        study = ctg["study"]
        summary = study["summary"]

        corte_carga_medio = summary["corte_carga_medio_MW"]
        curtailment_medio = summary["corte_vento_medio_MW"]

        MVu = np.array(summary["MVu_MW"], dtype=float)
        MVd = np.array(summary["MVd_MW"], dtype=float)

        rows_scalar.append({
            "CTG": f"Linha {line_id}",
            "corte_carga_medio_MW": corte_carga_medio,
            "curtailment_medio_MW": curtailment_medio,
            "MVu_max_MW": float(np.max(MVu)) if len(MVu) > 0 else 0.0,
            "MVd_max_MW": float(np.max(MVd)) if len(MVd) > 0 else 0.0,
        })

        row_up = {"CTG": f"Linha {line_id}"}
        row_down = {"CTG": f"Linha {line_id}"}

        for i, val in enumerate(MVu, start=1):
            row_up[f"G{i}"] = float(val)

        for i, val in enumerate(MVd, start=1):
            row_down[f"G{i}"] = float(val)

        rows_mvu_gen.append(row_up)
        rows_mvd_gen.append(row_down)

    df_ctg_scalar = pd.DataFrame(rows_scalar)
    df_ctg_mvu_gen = pd.DataFrame(rows_mvu_gen)
    df_ctg_mvd_gen = pd.DataFrame(rows_mvd_gen)

    # Matrizes pedidas individualmente
    df_ctg_corte = df_ctg_scalar[["CTG", "corte_carga_medio_MW"]].copy()
    df_ctg_curt = df_ctg_scalar[["CTG", "curtailment_medio_MW"]].copy()
    df_ctg_mvu = df_ctg_scalar[["CTG", "MVu_max_MW"]].copy()
    df_ctg_mvd = df_ctg_scalar[["CTG", "MVd_max_MW"]].copy()

    return {
        "df_ctg_scalar": df_ctg_scalar,
        "df_ctg_corte": df_ctg_corte,
        "df_ctg_curt": df_ctg_curt,
        "df_ctg_mvu": df_ctg_mvu,
        "df_ctg_mvd": df_ctg_mvd,
        "df_ctg_mvu_gen": df_ctg_mvu_gen,
        "df_ctg_mvd_gen": df_ctg_mvd_gen,
    }

def print_ctg_final_tables(ctg_study):
    tables = build_ctg_final_tables(ctg_study)

    print("\n" + "=" * 90)
    print("MATRIZ FINAL - CTG x CORTE DE CARGA MÉDIO")
    print("=" * 90)
    print(tables["df_ctg_corte"].to_string(index=False))

    print("\n" + "=" * 90)
    print("MATRIZ FINAL - CTG x CURTAILMENT MÉDIO")
    print("=" * 90)
    print(tables["df_ctg_curt"].to_string(index=False))

    print("\n" + "=" * 90)
    print("MATRIZ FINAL - CTG x MVu")
    print("=" * 90)
    print(tables["df_ctg_mvu"].to_string(index=False))

    print("\n" + "=" * 90)
    print("MATRIZ FINAL - CTG x MVd")
    print("=" * 90)
    print(tables["df_ctg_mvd"].to_string(index=False))

    print("\n" + "=" * 90)
    print("MATRIZ FINAL - CTG x MVu POR GERADOR")
    print("=" * 90)
    print(tables["df_ctg_mvu_gen"].to_string(index=False))

    print("\n" + "=" * 90)
    print("MATRIZ FINAL - CTG x MVd POR GERADOR")
    print("=" * 90)
    print(tables["df_ctg_mvd_gen"].to_string(index=False))
...
def plot_load_shedding_and_curtailment(study, ctg_label="CTG"):
    results = study["results"]
    ...
    fig.update_layout(
        title=f"Corte de Carga e Curtailment por Cenário • {ctg_label}",
        xaxis_title="Cenário",
...
def plot_top_ramps_combined(study, ctg_label="CTG", top_n=10, min_ramp_mw=1.0):
    results = study["results"]
    ...
    # seleciona apenas geradores com rampa expressiva
    selected_generators = []
    for g in range(nger):
        max_up = max(r["ramp_up_MW"][g] for r in results)
        max_down = max(r["ramp_down_MW"][g] for r in results)

        if max(max_up, max_down) >= min_ramp_mw:
            selected_generators.append(g)

    if len(selected_generators) == 0:
        print(f"{ctg_label}: nenhum gerador com rampa expressiva (>= {min_ramp_mw} MW).")
        return
    ...
    fig.update_layout(
        title=f"Ranking das Maiores Rampas por Gerador • {ctg_label}",
        template="plotly_white",
        barmode="group",
        height=900,
        width=1800,
...
def plot_ramps_by_scenario(study, ctg_label="CTG", mode="up", min_ramp_mw=1.0):
    results = study["results"]
    ...
    if mode == "up":
        titulo = f"Rampas de Subida por Cenário • {ctg_label}"
        ylab = "Ramp Up (MW)"
    else:
        titulo = f"Rampas de Descida por Cenário • {ctg_label}"
        ylab = "Ramp Down (MW)"
...
def plot_load_vs_generation(study, ctg_label="CTG"):
    results = study["results"]
    ...
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=cenarios, y=carga_total, mode="lines+markers", name="Carga Total (MW)"))
    fig.add_trace(go.Scatter(x=cenarios, y=geracao_real, mode="lines+markers", name="Geração Convencional Real (MW)"))
    fig.add_trace(go.Scatter(x=cenarios, y=eolica_disponivel, mode="lines+markers", name="Eólica Disponível (MW)"))
    fig.add_trace(go.Scatter(x=cenarios, y=eolica_utilizada, mode="lines+markers", name="Eólica Utilizada (MW)"))
    fig.add_trace(go.Scatter(x=cenarios, y=geracao_total, mode="lines+markers", name="Geração Total do Modelo (MW)"))
    fig.update_layout(
        title=f"Carga e Geração por Cenário • {ctg_label}",
        xaxis_title="Cenário",
        yaxis_title="Potência (MW)",
        template="plotly_white",
        height=900,
...
def select_ctg_lines_from_base_case(data, top_n=15, use_base_wind=True, weights=None):
    """
    Seleciona automaticamente as linhas mais críticas para contingência
    a partir do caso base.

    Critérios:
    - loading relativo = |fluxo| / limite
    - perdas da linha
    - lambda da restrição de fluxo

    Retorna:
    - ctg_line_order: lista 1-based das linhas escolhidas
    - ranking_df: DataFrame completo com score e métricas
    """
    ...
    PLOAD_base = data["PLOAD_b"].copy()

    if data["NGW"] > 0:
        if use_base_wind:
            # usa vento máximo disponível como caso base "com eólica"
            PGW_base = data["PGWMAX"].copy()
        else:
            # usa vento nulo para caso base conservador
            PGW_base = np.zeros(data["NGW"])
    else:
        PGW_base = np.zeros(0)

    sol = solve_opf_scenario(data, PLOAD_base, PGW_base, tol=TOL, max_iter=MAX_ITER)

    fluxo = np.abs(sol["fluxo"]) * PB
    perdas = sol["perdas_linha"] * PB
    limites = data["FLIM"] * PB

    # evitar divisão por zero
    limites_safe = np.where(limites <= 1e-9, 1e-9, limites)

    loading = fluxo / limites_safe

    if sol["lbd_cv"] is not None:
        lambdas = np.abs(sol["lbd_cv"])
    else:
        lambdas = np.zeros(data["NLIN"])

    # normalizações
    loading_n = loading / max(np.max(loading), 1e-9)
    loss_n = perdas / max(np.max(perdas), 1e-9)
    lambda_n = lambdas / max(np.max(lambdas), 1e-9)

    score = ( weights["loading"] * loading_n + weights["lambda"] * lambda_n + weights["loss"] * loss_n )

    ranking_df = pd.DataFrame({
        "line_id": np.arange(1, data["NLIN"] + 1),
        "from_bus": data["BUSID"][data["SB"]],
        "to_bus": data["BUSID"][data["EB"]],
        "flow_MW_abs": fluxo,
        "limit_MW": limites,
    ...
    ctg_line_order = ranking_df["line_id"].head(top_n).astype(int).tolist()

    return ctg_line_order, ranking_df
...
if __name__ == "__main__":
    ...
    print("\n============================")
    print("INICIANDO ANÁLISE DE CONTINGÊNCIA (CTG)")
    print("============================\n")

    # ordem das linhas contingenciadas
    # Para os sistema B6L8:
    # CTG_LINE_ORDER = [1, 3, 5]
    
    # Para o sistema IEEE118:
    # CTG_LINE_ORDER = [ 65, 68, 81, 38, 30, 37, 23, 25, 26, 45, 69, 75, 42, 8, 9 ]
    CTG_LINE_ORDER = [ 96, 37, 103 ]
    
    """
    CTG_LINE_ORDER, ranking_df = select_ctg_lines_from_base_case(
        data,
        top_n=2,
        use_base_wind=True
    )   
    ...
    print("\nCTG_LINE_ORDER automático:")
    print(CTG_LINE_ORDER)

    #print("\nTop 15 linhas mais críticas no caso base:")
    #print(ranking_df.head(15).to_string(index=False))

    # roda CTG
    ctg_study = run_contingency_study(
        DBAR=DBAR,
        DLIN=DLIN,
        DGER=DGER,
        DGW=DGW,
        ...
        Nc=NC,
        seed=123,
        line_order=CTG_LINE_ORDER
    )
    print("\nAnálise de contingência concluída com sucesso!")

    # ======================================================
    # SUMMARY GERAL DAS CTGs
    # ======================================================

    print_contingency_summary(ctg_study, top_n=10)
    print_ctg_final_tables(ctg_study)
    
    # salva excel consolidado
    filename = DATA_DIR / (
        f"CTG_results_"
        f"{sum(ctg_study['ctg_results'][0]['summary']['limites_MW']):.0f}_"
        f"{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
    )

    export_contingency_study_to_excel(
        ctg_study,
        filename=filename
    )
    ...
    for ctg in ctg_study["ctg_results"]:

        line_id = ctg["ctg_linha"]

        print("\n")
        print("====================================")
        print(f"RESULTADOS PARA CTG DA LINHA {line_id}")
        print("====================================")
        ...
        # cenários extremos
        print_scenario_extremes(study, top_n=5)

        # dashboard rápido
        print_results_dashboard(study)

        # rampas
        # print_top_ramps_per_generator(study, top_n=5)
        
        # Exporta excel específico do CTG
        filename_ctg = DATA_DIR / (
            f"CTG_Linha{line_id}_results_"
            f"{sum(ctg_study['ctg_results'][0]['summary']['limites_MW']):.0f}_"
            f"{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
        )
        ...
        plot_top_ramps_combined(study, ctg_label=f"CTG linha {line_id}", top_n=10)

        plot_ramps_by_scenario(study, ctg_label=f"CTG linha {line_id}", mode="up")

        plot_ramps_by_scenario(study, ctg_label=f"CTG linha {line_id}", mode="down")

        plot_load_shedding_and_curtailment(study, ctg_label=f"CTG linha {line_id}")

        plot_load_vs_generation(study, ctg_label=f"CTG linha {line_id}")

        plot_line_ranking_by_lambda(study, ctg_label=f"CTG linha {line_id}")
    
    print("\n")
    # Exportar ctg_study para pickle e entender a lógica de armazenamento e leitura depois
    
    joblib.dump(ctg_study, DATA_DIR / "ctg_study.pkl", compress=3)
    print("Objeto ctg_study exportado para ctg_study.pkl")
    
    with open(DATA_DIR / "ctg_study_v2.pkl", "wb") as f:
        pickle.dump(ctg_study, f, protocol=4)